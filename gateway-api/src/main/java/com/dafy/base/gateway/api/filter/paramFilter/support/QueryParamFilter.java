 ...
 */
@Component
public class QueryParamFilter implements IBodyDataFilter, Closeable {

    private final IParamFilter nextFilter;

    @Autowired
    public QueryParamFilter(@Qualifier("bodyParamFilter") IParamFilter nextFilter) {
        this.nextFilter = nextFilter;
    }

    @Override
    public String getAppKey(HttpServletRequest request) throws IOException {
        String value = request.getParameter(GatewayParamEnum.APP_KEY.getKey());
    ...
    public String getUrl(HttpServletRequest request) throws IOException {
        String value = request.getParameter(GatewayParamEnum.URL.getKey());
